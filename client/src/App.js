...
export default function App() {
  ...
  window.addEventListener("beforeunload", () =>
    remember ? null : localStorage.clear()
  )
  ...
  const signOut = async () => {
    ...
    axios.post(
      "http://18.193.224.15:8080/users-api/sign-out",
      {},
      {
        headers: {
          Authorization: token,
  ...
  const isLogged = async () => {
    ...
    if (token) {
      axios
        .get("http://18.193.224.15:8080/tenants-api/tenants", {
          headers: {
            Authorization: token,
          },
        })
  ...
  const handleClick = () => {
    ...
    if (value && value.length < 40 && value.length > 0) {
      ...
      if (value === "all") {
        ...
        const filterd = tenantsData.filter(
          (f) =>
    ...
    <>
      ...
      {isSignd ? (
        <>
          ...
          <span id={hint ? "hint" : "hidden"}>
            Search 'all' for all tenants information
          </span>
