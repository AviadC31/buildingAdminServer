  ...
  const toggleEdit = async (value) => {
    setEditing(!editing)
      if(record._id && editing){
        axios.put(`http://18.193.224.15:8080/tenants-api/tenant/${record._id}`,{
          field: dataIndex, 
...
export default function TableSection({ dataSource, liveData }) {
  ...
  //Removing tenant from DB by phone number (unique field)
    const handleDelete = (record) => {
      axios.delete(`http://18.193.224.15:8080/tenants-api/tenant/${record.phone}`)
  ...
  const handleSave = (row) => {
    ...
    if( 
      ...
      axios.post('http://18.193.224.15:8080/tenants-api/tenant', row)
